    ...
    if (container.find('.sc_googlemap:not(.inited)').length > 0) {
        container.find('.sc_googlemap:not(.inited)')
            .each(function() {
                ...
                dairy_farm_googlemap_init(jQuery('#' + map_id).get(0), {
                    style: map_style,
                    zoom: map_zoom,
                    markers: map_markers
                });
            ...
            .on('click', '.sort', function(e) {
                ...
                jQuery.post(DAIRY_FARM_STORAGE['ajax_url'], {
                    ...
                    try {
                        ...
                        rez = {
                            error: DAIRY_FARM_STORAGE['ajax_error']
                        };
...
function dairy_farm_sc_init_scroll_area(obj) {
    ...
    if (!dairy_farm_check_images_complete(obj) && DAIRY_FARM_STORAGE['scroll_init_counter']++ < 30) {
        setTimeout(function() {
            dairy_farm_sc_init_scroll_area(obj);
        }, 200);
        return;
    }
        ...
        var o = {
            ...
            diagram: function() {
                ...
                r.circle(c, c, Math.round(w / 2)).attr({
                    stroke: 'none',
                    fill: DAIRY_FARM_STORAGE['theme_bg_color'] ? DAIRY_FARM_STORAGE['theme_bg_color'] : '#ffffff'
                });
    ...
    if (!form_custom) {
        ...
        if (form.find('[name="username"]').length > 0) {
            rule = {
                ...
                max_length: {
                    value: 60,
                    message: DAIRY_FARM_STORAGE['strings']['name_long']
                }
            };
            if (form.find('[name="username"][aria-required="true"]').length > 0)
                rule['min_length'] = {
                    value: 1,
                    message: DAIRY_FARM_STORAGE['strings']['name_empty']
                };
            rules.push(rule);
        }
        ...
        if (form.find('[name="email"]').length > 0) {
            rule = {
                ...
                max_length: {
                    value: 60,
                    message: DAIRY_FARM_STORAGE['strings']['email_long']
                },
                mask: {
                    value: DAIRY_FARM_STORAGE['email_mask'],
                    message: DAIRY_FARM_STORAGE['strings']['email_not_valid']
                }
            };
            if (form.find('[name="email"][aria-required="true"]').length > 0)
                rule['min_length'] = {
                    value: 7,
                    message: DAIRY_FARM_STORAGE['strings']['email_empty']
                };
            rules.push(rule);
        }
        ...
        if (form.find('[name="subject"]').length > 0) {
            rule = {
                ...
                max_length: {
                    value: 100,
                    message: DAIRY_FARM_STORAGE['strings']['subject_long']
                }
            };
            if (form.find('[name="subject"][aria-required="true"]').length > 0)
                rule['min_length'] = {
                    value: 1,
                    message: DAIRY_FARM_STORAGE['strings']['subject_empty']
                };
            rules.push(rule);
        }
        ...
        if (form.find('[name="message"]').length > 0) {
            rule = {
                field: "message",
                max_length: {
                    value: DAIRY_FARM_STORAGE['contacts_maxlength'],
                    message: DAIRY_FARM_STORAGE['strings']['text_long']
                }
            };
            if (form.find('[name="message"][aria-required="true"]').length > 0)
                rule['min_length'] = {
                    value: 1,
                    message: DAIRY_FARM_STORAGE['strings']['text_empty']
                };
            rules.push(rule);
        }
...
function dairy_farm_menuitems_show_popup(obj) {
    ...
    jQuery.post(DAIRY_FARM_STORAGE['ajax_url'], {
        ...
        try {
            ...
            rez = {
                error: DAIRY_FARM_STORAGE['ajax_error'] + '<br>' + response
            };
        }
